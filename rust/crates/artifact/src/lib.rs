...
//!
//! The [`runtime`] crate builds an in-memory tree of `conformance.*` spans (see
...
//! # What is emitted
//!
//! A single [`TraceCapture`] renders to one `TracesData` object:
//! `resourceSpans[0]` carries the [`Resource`] attributes and one `scopeSpans`
...
//!
//! [`runtime::Value`] maps to OTLP `AnyValue` per CTSC §8: integers and
...
//! # Formats
//!
//! [`OtlpFormat::Json`] renders one pretty-printed `TracesData` object;
//! [`OtlpFormat::Jsonl`] renders the compact streaming form (one complete
